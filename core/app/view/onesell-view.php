...
<h1><i class="bi bi-receipt"></i> Resumen de Venta</h1>
<div class="mb-3">
  <a href="report/onesell-pdf.php?id=<?php echo $_GET["id"]; ?>" target="_blank" class="btn btn-success text-white"><i class="bi bi-download"></i> Descargar PDF</a>
...
<div class="card shadow-sm border-0 mb-4">
	<div class="card-header bg-white py-3">
		<h5 class="card-title mb-0 text-primary fw-bold"><i class="bi bi-info-circle me-2"></i>Información General de la Venta #<?php echo $sell->id; ?></h5>
	</div>
	<div class="card-body">
		<div class="row">
			...
			<div class="col-md-6">
				<h6 class="text-uppercase fw-bold text-muted small mb-3">Información del Cliente</h6>
				...
				<table class="table table-borderless table-sm mb-0">
					<tr>
						...
						<td><strong><?php echo $client->name." ".$client->lastname;?></strong></td>
					</tr>
					<?php if($client->phone1 != ""): ?>
					<tr>
						<td class="fw-bold text-secondary">Teléfono:</td>
						<td><?php echo $client->phone1; ?></td>
					</tr>
					<?php endif; ?>
					<?php if($client->email1 != ""): ?>
					<tr>
				...
				<?php else: ?>
					<p class="text-muted mb-0"><i class="bi bi-people me-2"></i>Público General</p>
				<?php endif; ?>
			</div>
		</div>
	</div>
...
<div class="card shadow-sm border-0 mb-4">
	<div class="card-header bg-white py-3">
		<h5 class="card-title mb-0 text-primary fw-bold"><i class="bi bi-box-seam me-2"></i>Productos Vendidos</h5>
	</div>
			...
			<table class="table table-bordered table-hover table-sm mb-0">
				...
				<tbody>
					<?php foreach($operations as $operation):
						...
						$subtotal_item = $operation->q * $product->price_out;
						$total += $subtotal_item;
					?>
					<tr>
			...
			<div class="card-body p-0">
				<table class="table table-borderless mb-0">
					...
					<tr class="border-bottom text-danger">
						<td class="px-3 py-2 fw-bold">Descuento:</td>
						<td class="px-3 py-2 text-end font-monospace">-$ <?php echo number_format($sell->discount, 2); ?></td>
					</tr>
					<tr class="border-bottom bg-light fs-5">
						<td class="px-3 py-2 fw-bold text-primary">Total:</td>
						<td class="px-3 py-2 text-end fw-bold text-primary font-monospace">$ <?php echo number_format($total - $sell->discount, 2); ?></td>
					</tr>
					...
					<tr class="text-info">
						...
						<td class="px-3 py-2 text-end fw-bold font-monospace">$ <?php echo number_format($sell->cash - ($total - $sell->discount), 2); ?></td>
					</tr>
					<?php endif; ?>
				</table>
			</div>
...
<?php else: ?>
	<div class="alert alert-danger">
		<i class="bi bi-exclamation-triangle-fill me-2"></i> Error 501: Identificador de venta no especificado.
	</div>
<?php endif; ?>
